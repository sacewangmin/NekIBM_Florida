...
c-----------------------------------------------------------------------
      ...
      parameter(rgrav = 100.0) ! gravitational acceleration
      ...
      ! uncoupled gravity in -y direction and buoyancy 
         ...
         lpmforce(3) = 0.0
         write(6,'(A,G12.6)')"Gravity Force = ", lpmforce(2)
      else
         lpmforce(1) = 0.0  
         lpmforce(2) = 0.0
         lpmforce(3) = 0.0
...
c-----------------------------------------------------------------------
      ...
      integer IDP
      common/ibm_debug_para/ibm_debug_parastudy
      ! for max error on particle velocity
      
      ibm_debug_parastudy = 1

      ifxyo=.true.
      if (istep.gt.1) ifxyo=.false.
      
c      if (istep .le. 50) part_force(1) = 4 ! particle drag
c      if (istep .gt. 10) part_force(1) = 5 ! ibm
      ...
      if(Periodic_Set.ge.1) open(unit=nitV,file="V_subtraction.txt")

      vf1=0.0167
      open(unit=9999999,file="particles.dat")
      read(9999999,*) num_of_IBMpart, xlow, xhigh,
     $               ylow, yhigh, zlow, zhigh
      x_length=(xhigh-xlow)*20.0
      y_length=(yhigh-ylow)*20.0
      z_length=(zhigh-zlow)*20.0
c      write(nitV,*) "Check periodic length: ",
c     $               x_length,y_length,z_length
c      num_of_IBMpart       = 2  ! particle number
c      Particle_shape       = 2
      ...
      ipart_stationary     = 0  ! stationary particles for debug only
      ipart_moving     	   = 1  ! stationary particles for debug only

      do nn = 1, num_of_IBMpart
         n_dh(nn) = 30
	 read(9999999,*) IDP, ibm_diam(nn), ibm_center(nn,1),
     $         ibm_center(nn,2), ibm_center(nn,3)
          ibm_diam(nn)=ibm_diam(nn)*20.0
          ibm_center(nn,1)=ibm_center(nn,1)*20.0
          ibm_center(nn,2)=ibm_center(nn,2)*20.0
          ibm_center(nn,3)=ibm_center(nn,3)*20.0
C        
c         write(nitV,*) nn, ibm_center(nn,1),
c     $          ibm_center(nn,2),ibm_center(nn,3)
!        read()
